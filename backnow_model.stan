...
data {
  ...
  // OBSERVED
  int<lower=0> N_obs;           // Number of individuals (can be zero)
  matrix[N_obs, J] dum_obs;     // matrix of indicator values
  array[N_obs] int<lower=1> Y_obs;    // observed reporting delays
  array[N_obs] int<lower=1> ReportOnset; // which days did these occur on ...

  // MISSING
  int<lower=1> N_miss;        // Number of individuals with missing data
  matrix[N_miss, J] dum_miss; // matrix of indicator values
  array[N_miss] int<lower=1> ReportDays; // which days were things reported on

  // Tells you the true order of cases
  array[N_obs + N_miss] int<lower=0, upper=1> missvector;
}
...
transformed parameters {
  vector[N_obs] mu_obs = exp(dum_obs * betas);
  vector[N_miss] mu_miss = exp(dum_miss * betas);
}
...
generated quantities {
  ...
  // (1) Create vectors for onset days (allOnset) and reporting delays (allY)
